...
function addProduct(id){
  ...
  const exist=addCart.some((prod)=> prod.id===id)
    if(exist){
    addCart.map(prod=>{if(prod.id===id){
       prod.cantidad++
    }})

  }
  else{
     let addItem= products.find((aProd)=>   // ACA FIGURA Q NO ENCUENTRA "products"
  aProd.id===id)
...
let total=addCart.reduce((acc,prod)=>acc + prod.cantidad*prod.precio, 0)
...
saveStorage()
 ...
 function removeProduct(id){          // ME MARCA COMO QUE NO SE USA
  const idItem=id
...
saveStorage()
 ...
 function finishOrder(){  // ME MARCA COMO QUE NO SE USA
